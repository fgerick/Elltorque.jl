...
addprocs_lsf(parse(Int,ARGS[1]))
@everywhere using Elltorque, DoubleFloats, JLD2, GenericSchur
@show nprocs()
datapath = ARGS[2]
...
@sync @distributed for i in 1:length(les)
    RHSt = copy(RHS)
    Mire.projectforce!(view(RHSt,1:length(vs2),1:length(vs2)),cmat,vs2,vs2,coriolis,[0,0,1/les[i]])
    # esol = eigen(invL*RHSt)
    esol = schur(complex.(Matrix(RHSt)),complex.(Matrix(LHS)))
    inds = 1e-2.<abs.(esol.values).<1e2
    ωs = esol.values[inds]
    evecs = eigvecs(esol)
    us,bs,eks,ebs = Elltorque.get_ub(evecs[:,inds],vs1,vs2,cmat; ekin=false, getenergies=true)
