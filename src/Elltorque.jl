...
function run(calculate=false)
    ...
    # @show pwd()
    PyPlot.pygui(false)
    include(joinpath(ellpath,"convergence.jl"))
    println("convergence done.")
    include(joinpath(ellpath,"torques.jl"))
    println("torques done.")
    include(joinpath(ellpath,"dense_angularmomentum.jl"))
    println("dense angular momentum done.")
    include(joinpath(ellpath,"dense_sphere3d.jl"))
    println("dense sphere done.")
    include(joinpath(ellpath,"ellipticity.jl"))
    println("ellipticity done.")
    close("all")
    include(joinpath(ellpath,"lehnert.jl"))
